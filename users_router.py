...
from databases_functions import insert_sex_find, insert_age_find, insert_media, insert_type, print_registration_profile, \
    insert_description, not_in_database, select_name, insert_full_name, insert_sex, insert_age
from reply import del_keyboard, start_keyboard, find_sex_keyboard, sex_keyboard

user_router = Router()
connection_pool = get_connection_pool()


@user_router.message(Command("profile"), F.text)
...
@user_router.message(User.registration, F.text)
...
@user_router.message(User.name, F.text)
...
@user_router.message(User.sex, F.text)
...
@user_router.message(User.find_sex, F.text)
...
@user_router.message(User.age, F.text)
...
@user_router.message(User.age_find, F.text)
...
@user_router.message(User.image, F.content_type.in_([F.PHOTO, F.VIDEO]))
...
@user_router.message(User.wait, F.text)
